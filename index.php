			...
			el('productList').innerHTML = listItems.join('');
							...
							myLib.get({action:'cat_fetchall'}, function(json){
								...
								for (var listItems = [],
										i = 0, cat; cat = json[i]; i++) {
									if(cat.catid==id){
										listItems.push( '  >>>  ','<a href="index.php?catid=',id,'">',cat.name.escapeHTML(),'</a>');
									}
								}
								el('navigationMenu').innerHTML = listItems.join('');
							});
