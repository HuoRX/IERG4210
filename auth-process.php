...
function ierg4210_chgPass(){
	if (empty($_POST['email']) || empty($_POST['pw']) || empty($_POST['npw'])
		|| !preg_match("/^[\w=+\-\/][\w='+\-\/\.]*@[\w\-]+(\.[\w\-]+)*(\.[\w]{2,6})$/", $_POST['email'])
		|| !preg_match("/^[\w@#$%\^\&\*\-]+$/", $_POST['pw'])
		|| !preg_match("/^[\w@#$%\^\&\*\-]+$/", $_POST['npw']))
		throw new Exception('Wrong Credentials');
		//header('Location: login.php');

	// Implement the login logic here
	$login_success=loginProcess($_POST['email'],$_POST['pw']);
	 //login process is in auth.php

	if ($login_success){
		//prevent session fixation
		chgProcess($_POST['email'],$_POST['npw']);
