...
function chgProcess($email, $password){
	$db=newDB();
		$q=$db->prepare('SELECT * FROM accounts WHERE email = ?');
		$q->execute(array($email));
		if($r=$q->fetch()){
...
function loginstate()
{
	...
	if(!empty($_SESSION['nortoken']))
		return $_SESSION['nortoken']['em'];
	if(!empty($_COOKIE['nortoken'])){
		//stripslashes() Returns a string with backslashes stripped off. // (\' becomes ' and so on.)
		if($t = json_decode(stripslashes($_COOKIE['nortoken']),true)){
			if (time() > $t['exp'])
				return false; // to expire the user
			$db=newDB();
			$q=$db->prepare('SELECT * FROM account WHERE email = ?');
