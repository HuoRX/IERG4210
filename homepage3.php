...
<section id="categoryListPanel">
	...
	<section id ="userinfo">
		...
		<form id="logout" method="POST" action="auth-process.php?action=logout">
			...
			<div>
			<input type="text" name="email" size="20" value="<?php echo loginstate(); ?>" />
		  </div>
		...
		for (var listItems = [],
				i = 0, prod; prod = json[i]; i++) {
					if(localStorage.getItem(parseInt(prod.pid))===null){}
						...
						listItems.push('<input type="hidden" name="quantity_',j+1,'" value="'+localStorage.getItem(parseInt(prod.pid)).escapeHTML()+'"/>');
